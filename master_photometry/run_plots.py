...
if __name__ == '__main__':
    ...
    ax.errorbar(kinematics['P_turb'],SFR_n_Gas['SFR'][indices],xerr=kinematics['P_turb_err'],yerr=SFR_n_Gas['SFR_error'][indices], fmt='.', color ='gray', alpha=.2)
    ax.set_xscale("log")
    ax.set_yscale("log")
    ...
    print('Fitting the k-S slope...')

    x_data = np.log10(np.array(SFR_n_Gas['$\u03A3$_H2'][indices]))
    y_data = np.log10(np.array(SFR_n_Gas['$\u03A3$_SFR'][indices]))
    x_err = np.array(SFR_n_Gas['$\u03A3$_H2_err'][indices])
    y_err = np.array(SFR_n_Gas['$\u03A3$_SFR_err'][indices])

    y_val_max = np.max(y_data)
    y_val_min = np.min(y_data)
    x_val_max = np.max(x_data)
    x_val_min = np.min(x_data)

    y_data_err = (1/np.log(10))*y_err/np.power(10,y_data)
    x_data_err = (1/np.log(10))*x_err/np.power(10,x_data)
    lm = linmix.LinMix(x_data, y_data, x_data_err, y_data_err, K=2)
    lm.run_mcmc(silent=True)


    print('max slope     ',np.max(lm.chain['beta']))
    print('min slope     ',np.min(lm.chain['beta']))
    print('######')
    print('median slope  ',np.median(lm.chain['beta']))
    print('median alpha   ',np.median(lm.chain['alpha']))
    print('median sigsqr ',np.median(lm.chain['sigsqr']))
    ...
    ax.errorbar(x_data, y_data, xerr=xsig, yerr=ysig, ls=' ', alpha=0.2)


    for i in range(0, len(lm.chain), 25):
        xs = np.arange(x_val_min-2,x_val_max+2)
        ys = lm.chain[i]['alpha'] + xs * lm.chain[i]['beta']
        ax.plot(xs, ys, color='r', alpha=0.02)
    ys = alpha + xs * beta
    ax.plot(xs, ys, color='k', label = 'Fit with \n slope ='+ str('%.2g' % beta)+ '$\pm$'+ str('%.1g' % np.sqrt(sigsqr)))

    plt.scatter(np.log10(SFR_n_Gas['$\u03A3$_H2'][indices]),np.log10(SFR_n_Gas['$\u03A3$_SFR'][indices]),color='k',marker = 's',facecolors='gray',s=200 ,label = 'Data '+name)

    plt.ylabel('Log $\u03A3_{SFR}$[$M_{\odot}$ $y^{-1}$ $kpc^{-2}$]',fontsize=40)
    plt.xlabel('Log $\u03A3_{Mol}$[$M_{\odot}$ $pc^{-2}$]',fontsize=40)

    plt.yticks(fontsize=40)
    plt.xticks(fontsize=40)
    ax.legend(fontsize=30)

    d_y = np.abs(0.3*(y_val_max-y_val_min))
    d_x = np.abs(0.3*(x_val_max-x_val_min))

    ax.set_ylim(y_val_min - d_y, y_val_max + d_y)
    ax.set_xlim(x_val_min - d_x, x_val_max + d_x)
    ax.minorticks_on()
    ax.tick_params('both', length=15, width=2, which='major')
    ax.tick_params('both', length=8, width=1, which='minor')
    plt.savefig(parent_dir + '/' + new_folder + '/' + 'K-S_slope.png',bbox_inches='tight')
    plt.close()
